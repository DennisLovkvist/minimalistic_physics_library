...
void mpl_debug_draw_polygon(PrimitiveBatch *primitive_batch,unsigned int shader_primitive,Projection projection, Polygon *polygon)
{
    delo2d_primitive_batch_begin(primitive_batch,shader_primitive,projection,DELO_TRIANGLE_LIST);
        ...
        {
            delo2d_primitive_batch_add(primitive_batch,polygon->vertices[0].x,polygon->vertices[0].y,1,1,1,0.5);
            delo2d_primitive_batch_add(primitive_batch,polygon->vertices[1].x,polygon->vertices[1].y,1,1,1,0.5);
            delo2d_primitive_batch_add(primitive_batch,polygon->vertices[2].x,polygon->vertices[2].y,1,1,1,0.5);
        }    
        else if(polygon->vertex_count == 4)
        {
            ...
            delo2d_primitive_batch_add(primitive_batch,polygon->vertices[2].x,polygon->vertices[2].y,1,1,1,0.5);
            delo2d_primitive_batch_add(primitive_batch,polygon->vertices[3].x,polygon->vertices[3].y,1,1,1,0.5);
            delo2d_primitive_batch_add(primitive_batch,polygon->vertices[0].x,polygon->vertices[0].y,1,1,1,0.5);
        } 
        else if(polygon->vertex_count == 0)
        {
            ...
            {               
                ...
                delo2d_primitive_batch_add(primitive_batch,polygon->position.x,polygon->position.y,1,1,1,0.5);

                delo2d_primitive_batch_add(primitive_batch,prev.x,prev.y,1,1,1,0.5);
                r += inc;
                float x = polygon->position.x + cos(r)*polygon->radius;
                float y = polygon->position.y + sin(r)*polygon->radius;
                delo2d_primitive_batch_add(primitive_batch,x,y,1,1,1,0.5);
